...
var Ren = function () {
    function Ren(cb) {
        ...
        //加载狐狸
        let path = this._config.Ren.path;
        let filename = this._config.Ren.name;
        ...
        this.loadOBJ = new LoadOBJ(path,filename,function (foxObj) {
            ...
            this.bottle.position.y = this._config.REN.bodyHeight / 2 + 0.5;
            cb && cb();
...
Ren.prototype = {
    ...
    //五彩石 +1 提示
    createColorStoneHint:function(){
        let material = new THREE.MeshBasicMaterial({
            map: loader.load('res/img/dalisi/addColorStone.png'),
            transparent: true,
            alphaTest: 0.01
        });
        let geometry = new THREE.PlaneGeometry(3, 1.8);
        let toast = new THREE.Mesh(geometry, material);
        toast.position.set(0,3,0);
        toast.rotation.y = -Math.PI / 4;
        toast.visible = false;
        this.colorStoneHint = toast;
        this.obj.add(toast);
    },
    ...
    showAddLecard:function(){
        if (this.leCardTint){
            ...
            (0, this._animation.TweenAnimation)(this.leCardTint.position.y, this._config.REN.bodyHeight + 6, 800, function (value) {
    ...
    showAddColorStone:function(){
        if (this.colorStoneHint){
            let self = this;
            this.changeScorePos(5);
            this.colorStoneHint.visible = true;
            this.colorStoneHint.position.y = 5;
            this.colorStoneHint.material.opacity = 1;
            (0, this._animation.TweenAnimation)(this.colorStoneHint.position.y, this._config.REN.bodyHeight + 6, 800, function (value) {
                this.colorStoneHint.position.y = value;
            }.bind(this));
            (0, this._animation.TweenAnimation)(this.colorStoneHint.material.opacity, 0, 800, function (value, complete) {
                this.colorStoneHint.material.opacity = value;
                if (complete) {
                    this.colorStoneHint.visible = false;
                    this.changeScorePos(0);
                }
            }.bind(this));
        }
    ...
    addScore.rotation.y = -Math.PI / 2;
        ...
        (0, this._animation.TweenAnimation)(this.scoreText.obj.position.y, this._config.REN.bodyHeight + 6, 800, function (value) {
    ...
    _scatterParticles: function (particle) {
        let that = this;
        let minDistance = this._config.REN.bodyWidth / 2;
        let maxDistance = 2;
    ...
    _prepare: function () {
        ...
        this.scale -= this._config.REN.reduction;
        this.scale = Math.max(this._config.REN.minScale, this.scale);
        if (this.scale <= this._config.REN.minScale) {
            return;
        }
        this.body.scale.y = this.scale;
        this.body.scale.x += 0.002;
        this.body.scale.z += 0.002;
        this.head.position.y -= 0.006;
        let distance = 0.027;
    ...
    // 人变身
    bianshen:function(direction,cb){
        if (direction === 'straight'){
            // 沿x轴旋转
            this.bottle.rotation.y = 0;
            this.human.rotation.x = -Math.PI;
            (0, this._animation.TweenAnimation)(this.human.rotation.x, 0, 300, function (value,compete) {
            ...
            // 沿z轴旋转
            this.bottle.rotation.y = Math.PI/2;
            this.human.rotation.z = -Math.PI;
            (0, this._animation.TweenAnimation)(this.human.rotation.z, 0, 300, function (value,compete) {
    ...
    showup: function () {
        ...
        this.human.rotation.x = this.human.rotation.z = 0;
        (0, this._animation.TweenAnimation)(this.obj.position.y, this._config.BLOCK.height / 2+0.5, 500, 'Bounce.easeOut', function (value, complete) {
            this.obj.position.y = value;
            if (complete) {
                this.status = 'stop';
            }
    ...
    reset: function () {
        ...
        this.stop();
        this.currentSeason = 0;
        this.obj.position.y = this._config.BLOCK.height / 2+0.5;
