...
THREE.OBJLoader = ( function () {
    ...
    OBJLoader.prototype = {
        ...
        load: function ( url, onLoad, onProgress, onError ) {
            ...
            loader.load( url, function ( text ) {
                if (window.WechatGame){
                    //缓存到本地
                    // Utils.setStorageSync(scope.path+url,text);
                    wx.setStorage({
                        key:scope.path+url,
                        data:text
                    });
        ...
        //加载缓存到本地的obj内容
        loadLocalText:function(url,onLoad){
            if (window.WechatGame){
                wx.getStorage({
                    key:this.path+url,
                    success:function (res) {
                        onLoad(this.parse(res.data));
                    }.bind(this)
                });
            }
            // let text = Utils.getStorageSync(this.path+url);
            // onLoad(this.parse(text));
        },

        //加载包内obj文件
        loadLocal:function(url,onLoad,onError){
            var scope = this;
            if (window.WechatGame){
                var fileManager = wx.getFileSystemManager();
                fileManager.readFile({
                    filePath:this.path+url,
                    encoding:'utf8',
                    success:function (res) {
                        console.log('加载 本地obj 成功');
                        // console.log(res.data);
                        onLoad(scope.parse(res.data));
                    },
                    fail:function (res) {
                        console.log('加载 本地obj 失败');
                        console.log(res.errMsg);
                        onError(res.errMsg);
                    }
                });
            }else{
                onError('H5环境 加载本地obj 失败');
            }
        },
